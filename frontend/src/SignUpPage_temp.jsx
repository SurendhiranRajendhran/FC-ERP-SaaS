...
export default function SignUpPage() {
  ...
  const styles = {
    container: { backgroundColor: theme.bg, color: theme.textPrimary, fontFamily: 'system-ui, -apple-system, sans-serif', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.message || 'Failed to submit registration. Please try again.');
      }
      setSuccess(true);
    } catch (err) {
