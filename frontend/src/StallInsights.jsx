            ...
            <tbody>
              {ledgerItems.orders.map((o, idx) => (
                <tr key={idx} style={{ transition: 'background 0.2s' }}
                  ...
                  <td style={tableCellStyle}>#{o.order_id}</td>
                  <td style={tableCellStyle}>{o.order_date ? new Date(o.order_date).toLocaleDateString() : '-'}</td>
                  ...
                  <td style={tableCellStyle}>{o.item_name}</td>
                  <td style={tableCellStyle}>{o.quantity}</td>
                  <td style={tableCellStyle}>₹{parseFloat(o.price).toFixed(2)}</td>
                  <td style={{...tableCellStyle, color: '#ef4444', fontWeight: 600}}>₹{parseFloat(o.total_price || (o.quantity * o.price)).toFixed(2)}</td>
                </tr>
              ))}
              {ledgerItems.orders.length === 0 && (
                <tr><td colSpan="7" style={{...tableCellStyle, textAlign: 'center', color: '#64748b'}}>No unsettled items found</td></tr>
              )}
            </tbody>
            {ledgerItems.orders.length > 0 && (
              <tfoot>
                <tr style={{ background: 'rgba(239,68,68,0.05)' }}>
                  <td colSpan="6" style={{...tableCellStyle, textAlign: 'right', fontWeight: 600, color: '#fca5a5'}}>TOTAL PENDING</td>
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <StatCard
          ...
          value={`₹${expenses.reduce((s, e) => s + parseFloat(e.amount || 0), 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
          color="#ef4444"
          subtext={`${expenses.length} expense entries`}
        />
        {(() => {
          ...
          return (
            <StatCard
              ...
              color="#06b6d4"
              subtext={categories.slice(0, 3).join(', ') + (categories.length > 3 ? '...' : '')}
            />
          );
        })()}
      </div>
